...
const sections = ref<Section[]>([
  {
    title: undefined,
    content: `
Shortly after writing this document, I will be graduating with a Bachelor of Science in Computer Science from the University of British Columbia. One of my final credits was through directed studies, provided to me and overseen by Dr. Ramon Lawrence, with whom I had completed my capstone project.

This article will serve as a summary of the work I completed during my directed studies, and all the lessons learned.`,
  },
  {
...
There was an issue where the same student would show up in the notification over and over, so this was a quick fix by adding unique identifiers to already-displayed scores.

\`\`\`js
// Keep track of already-displayed scores
const displayedScores = new Set();

// In the score update function:
if (!displayedScores.has(scoreId)) {
    displayedScores.add(scoreId);
    showToast(\`\${username} increased their score by \${points} points!\`);
}
\`\`\``,
  },
  {
